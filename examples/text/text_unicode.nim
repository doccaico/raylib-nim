...
let
  emojiCodepoints: cstring = "\xF0\x9F\x8C\x80\x00\xF0\x9F\x98\x80\x00\xF0\x9F\x98\x82\x00\xF0\x9F\xA4\xA3\x00\xF0\x9F\x98\x83\x00\xF0\x9F\x98\x86\x00\xF0\x9F\x98\x89\x00" &
...
var
  emoji: array[EMOJI_PER_WIDTH*EMOJI_PER_HEIGHT, tuple[index: cint, message: cint, color: Color]]
  hovered = -1.cint
  selected = -1.cint
  fontDefault = LoadFont("resources/dejavu.fnt")
  fontAsian = LoadFont("resources/notoCJK.fnt")
  fontEmoji = LoadFont("resources/emoji.fnt")
  hoveredPos = Vector2(x: 0.0, y: 0.0)
  selectedPos =Vector2(x: 0.0, y: 0.0)

const emojiLen = emoji.len
const messagesLen = messages.len
...
while not WindowShouldClose():
  ...
  for i in 0..<emojiLen:
    let txt = emojiCodepoints[emoji[i].index].unsafeAddr
    let emojiRect = Rectangle(x: pos.x, y: pos.y,
        width: fontEmoji.baseSize.cfloat, height: fontEmoji.baseSize.cfloat)
    if not CheckCollisionPointRec(mouse, emojiRect):
      DrawTextEx(fontEmoji, txt, pos, fontEmoji.baseSize.cfloat, 1.0,
          if selected == i: emoji[i].color else: Fade(LIGHTGRAY, 0.4))
    else:
      DrawTextEx(fontEmoji, txt, pos, fontEmoji.baseSize.cfloat, 1.0, emoji[i].color)
      hovered = i.cint
      hoveredPos = pos
    if i != 0 and i mod EMOJI_PER_WIDTH == 0:
      pos.y += fontEmoji.baseSize.cfloat + 24.25
      pos.x = 28.8
    else:
      pos.x += fontEmoji.baseSize.cfloat + 28.8

  if selected != -1:
    let message = emoji[selected].message
    let horizontalPadding = 20
    let verticalPadding = 30
    var font = fontDefault.addr

    if TextIsEqual(messages[message].language, "Chinese") or
        TextIsEqual(messages[message].language, "Korean") or
        TextIsEqual(messages[message].language, "Japanese"):
      font = fontAsian.addr

    var sz = MeasureTextEx(font[], messages[message].text, font.baseSize.cfloat, 1.0)
    if sz.x > 300:
      sz.y *= sz.x/300
      sz.x = 300
    elif sz.x < 160:
    ...
    var msgRect = Rectangle(x: selectedPos.x - 38.8, y: selectedPos.y,
        width: (2*horizontalPadding).cfloat + sz.x, height: (2*verticalPadding).cfloat + sz.y)
    msgRect.y -= msgRect.height
    var a = Vector2(x: selectedPos.x, y: msgRect.y + msgRect.height)
    var b = Vector2(x: a.x + 8, y: a.y + 10)
    var c = Vector2(x: a.x + 10, y: a.y)
    if msgRect.x < 10:
      msgRect.x += 28
    if msgRect.y < 10:
    ...
    let
      textRect = Rectangle(x: msgRect.x + horizontalPadding/2, y: msgRect.y + verticalPadding/2,
          width: msgRect.width - horizontalPadding.cfloat, height: msgRect.height)
      size = messages[message].text.len
      length = GetCodepointsCount(messages[message].text)
      info = TextFormat("%s %u characters %i bytes", messages[message].language, length, size)
    DrawTextRec(font[], messages[message].text, textRect, font.baseSize.cfloat, 1.0, true, WHITE)
    sz = MeasureTextEx(GetFontDefault(), info, 10, 1.0)
    let pos = Vector2(x: textRect.x + textRect.width - sz.x, y: msgRect.y + msgRect.height - sz.y - 2)
    DrawText(info, pos.x.cint, pos.y.cint, 10, RAYWHITE)

  DrawText("These emojis have something to tell you, click each to find out!",
      ((screenWidth - 650)/2).cint, screenHeight - 40, 20, GRAY)
  DrawText("Each emoji is a unicode character from a font, not a texture... Press [SPACEBAR] to refresh",
      ((screenWidth - 484)/2).cint, screenHeight - 16, 10, GRAY)
...
proc RandomizeEmoji() =
  hovered = -1
  selected = -1
  var start = GetRandomValue(45, 360)
  for i in 0..<emojiLen:
    emoji[i].index = GetRandomValue(0, 179)*5
    var hsv = Vector3(x: ((start*(i + 1)) mod 360).cfloat, y: 0.6, z: 0.85)
    emoji[i].color = Fade(ColorFromHSV(hsv), 0.8)
    emoji[i].message = GetRandomValue(0, messagesLen - 1)
